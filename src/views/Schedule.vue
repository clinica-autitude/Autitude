...
import { SITE_CONFIG, AGE_LIMITS } from '../config.js'
...
export default {
  ...
  setup() {
    ...
    const whatsappLink = computed(() => {
      ...
      return `${SITE_CONFIG.whatsappUrl}?text=${text}`
    })
    ...
    const incrementAge = () => {
      const current = form.value.childAge || 0
      if (current < AGE_LIMITS.pediatric) form.value.childAge = current + 1
    }

    const decrementAge = () => {
      const current = form.value.childAge || 0
      if (current > 0) form.value.childAge = current - 1
    }

    const canIncrement = computed(() => (form.value.childAge || 0) < AGE_LIMITS.pediatric)
    const canDecrement = computed(() => (form.value.childAge || 0) > 0)

    const ageDisplay = computed(() => {
      const age = form.value.childAge || 0
    ...
    return {
      ...
      incrementAge, decrementAge, canIncrement, canDecrement, ageDisplay
    }
  }
}
</script>
...
.age-selector:focus-within {
  ...
  box-shadow: 0 0 0 4px rgba(167, 139, 250, 0.2), 0 4px 12px rgba(167, 139, 250, 0.15);
}
